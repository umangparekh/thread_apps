 ...
 */
#define INT16U_MAX ((unsigned short)(~0))
 ...
 * packet header.
 */
typedef EmberMessageBuffer PacketHeader;
...
#endif
{
  ...
  /** A device that communicates only via broadcasts using its long id.
   * It lacks short id, parent, and ability to route messages. */
  EMBER_LURKER = 6,
  EMBER_COMMISSIONER = 7,
  /** A router that can act as a parent to other nodes, as well as relay
   *  messages to legacy lurker devices.  The base node type is still 2,
   *  but with the top bit (known as the lurker bit) set. */
  EMBER_ROUTER_LURKER = 0x82
};
/**
 * @}
 */
...
#endif
{
  ...
  /** The node is joined without a parent. */

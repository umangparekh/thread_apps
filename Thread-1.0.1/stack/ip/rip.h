...
#define RIP_TABLE_SIZE (RIP_MAX_ROUTERS + RIP_MAX_LURKERS)
...
// Don't go back to the minimum unless we have gotten up to 12.
// This avoids clogging the channel when there is a lot of churn.
#define MIN_TRICKLE_EXP_TO_HASTEN 12
...
#define emNeighborAge(ripId) (emRipTable[(ripId)].flags & NEIGHBOR_AGE_MASK)
#define emUnreachabilityTimer(ripId) \
  ((emRipTable[(ripId)].flags & UNREACHABILITY_TIMER_MASK) >> UNREACHABILITY_TIMER_SHIFT)
...
uint8_t emRssiToQuality(int8_t rssi);
...
void emClearNeighbor(uint8_t index);
...
uint8_t emAssignRipId(uint8_t requestedRipId, const uint8_t *longId);

...
#include "app/ip-ncp/management.h"
...
static void connectSocket(uint16_t port)
{
  emberInitializeCommandState(&callbackCommandState);
                                  ...
                                  uint16_t length)
{
  ...
  if (messageType == UART_LINK_TYPE_MANAGEMENT) {
    ...
    uint16_t identifier = emberFetchHighLowInt16u(data + 2);
...
static bool checkInput(void)
{
  ...
  readIpv6Input(dataFd, &dataStream, emHostIpv6Dispatch);
#endif

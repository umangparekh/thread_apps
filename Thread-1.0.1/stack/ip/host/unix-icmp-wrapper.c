...
EmberStatus emberIcmpListen(const uint8_t *localAddress)
{
  uint16_t icmpPort = prefixToPort(localAddress);

  if (emIsUnspecifiedAddress(localAddress)) {
    return EMBER_ERR_FATAL;
  }
  if (emberFindListener(icmpPort, localAddress) != NULL) {
...
{
  uint8_t source[16];
  uint16_t icmpSourcePort = prefixToPort(destination);
  emStoreIpSourceAddress(source, destination);
  ...
  if (listener != NULL) {
    ...
    fprintf(stderr, "No ICMP listener for source\n");
    return false;
  }
}

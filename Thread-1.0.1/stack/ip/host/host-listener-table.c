...
void emberCloseListeners(void)
{
  ...
  for (i = 0; i < EMBER_HOST_LISTENER_TABLE_SIZE; i++) {
    ...
    listeners[i].socket = INVALID_SOCKET;
    listeners[i].port = 0;
    MEMSET(listeners[i].localAddress, 0, 16);
  }
}

HostListener *emberFindListener(uint16_t port, const uint8_t *localAddress)
{
  uint8_t i;
  for (i = 0; i < EMBER_HOST_LISTENER_TABLE_SIZE; i++) {
    if (port == 0) {
      ...
      if (listeners[i].socket != INVALID_SOCKET
          ...
          && (localAddress == NULL
              || memcmp(listeners[i].localAddress, localAddress, 16) == 0)) {
        return &listeners[i];
      }
    }
  }
                               ...
                               int protocol)
{
  ...
  if (listener != NULL) {
    listener->port = port;
    MEMCOPY(listener->localAddress, localAddress, 16);

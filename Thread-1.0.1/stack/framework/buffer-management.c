...
#define ISR_PROTECT_BIT 0x2000
#define SIZE_MASK       0x1FFF

// If ISR_PROTECT_BIT is set, then the object starts with a pointer to
// a location pointing back to the object.  Interrupts must be disabled
// when moving the object and the location must be updated before renabling
// interrupts.
//
...
#define setProtected(thing)   (sizeWord(thing) |= ISR_PROTECT_BIT)
#define clearProtected(thing) (sizeWord(thing) &= ~ISR_PROTECT_BIT)
#define isProtected(thing)    BOOLEAN((sizeWord(thing) & ISR_PROTECT_BIT))
...
// The pointer stored at the beginning of a protected buffer.
#define protectedRoot(thing) (*((uint16_t **) ((thing) + OVERHEAD_IN_WORDS)))

#ifdef EMBER_TEST
bool bufferIsProtected(Buffer buffer)
{
  return isProtected(expandPointer(buffer));
}
#endif
...
static void moveToNewLocation(uint16_t *old, uint16_t *new)
{
  ...
  if (isProtected(old)) {
    ATOMIC(
           ...
           *protectedRoot(old) = newLocation(old);
           );
  } else {
    MEMMOVE(new, old, size);
  }
...
void emSetBufferLengthFromEnd(Buffer buffer, uint16_t newLength)
{
  ...
  if (isIndirect(bufferPointer)) {
    *((const uint8_t **) (bufferPointer + INDIRECT_BUFFER_POINTER_INDEX))
    ...
    uint8_t *contents = emberLinkedBufferContents(buffer);
...
{
  EmberMessageBuffer buffer = emReallyAllocateBuffer(length, async);
...
{
  uint16_t *bufferPointer = expandPointer(*bufferLocation);
  assert(isHeapPointer(bufferPointer));
  setProtected(bufferPointer);
  protectedRoot(bufferPointer) = bufferLocation;
}
...
void emClearInterruptProtectBuffer(Buffer buffer)
{
  uint16_t *bufferPointer = expandPointer(buffer);
  assert(isHeapPointer(bufferPointer));
  clearProtected(bufferPointer);
  protectedRoot(bufferPointer) = NULL;          // for safety
}
...
{
  ATOMIC(emBufferQueueAdd(&phyToMacQueue, newTail);)
}
  ...
  for (i = 0; i < length; i++) {
    fprintf(stderr, "%c%02X",
            (i = 0
             ? '['
             : ' '),
            emberLinkedBufferContents(buffer)[j]);
  }
...
void emberCopyBufferBytes(EmberMessageBuffer to,
                          ...
                          uint16_t count)
{
  MEMMOVE(emberLinkedBufferContents(to) + toIndex,
          emberLinkedBufferContents(from) + fromIndex,
          count);
}

...
#if defined(EMBER_SERIAL_USE_STDIO)
  #include <stdio.h>
#elif defined(EMBER_STACK_COBRA)
  #include "hal_types.h"
  #include "hal_board.h"
  #include "hal/micro/c8051/cobra/uart-cobra.h"
...
{
  assert(port < EM_NUM_SERIAL_PORTS);
#ifdef EMBER_SERIAL_USE_STDIO
  while(length--) {
    uint8_t c = *data++;
    while (putchar(c) != c) {/*empty*/};  // retry until success
  }
  return EMBER_SUCCESS;
#else //EMBER_SERIAL_USE_STDIO
  switch(emSerialPortModes[port]) {
...
#ifdef EM_ENABLE_SERIAL_BUFFER // TODO: Update to new buffers.

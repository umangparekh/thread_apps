...
void emOpenNcpUart(const char *uartDevice)
{
  ...
  fcntl(driverNcpFd, F_SETFL, FNDELAY);
...
void emCheckNcpUartInput(fd_set *input)
{
  if (FD_ISSET(driverDataFd, input)) {
    IpModemReadStatus status =
      readIpv6Input(driverDataFd, &dataStream, dataHandler);
...
{
  emLogLine(IP_MODEM, "MGMT driver->ncp '%s'", command + 4);

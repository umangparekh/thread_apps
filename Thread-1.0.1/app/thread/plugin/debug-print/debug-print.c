...
// This function prints IPv6 fields per the recommendations of RFC5952.  In
// particular, leading zeros within a field are omitted and lowercase is used
// for the letters a through f.
static void printIpv6Field(uint16_t field)
{
  bool printing = false;
  uint8_t i;
  for (i = 0; i < 4; i++) {
    uint8_t nibble = (uint8_t)(((field << (i * 4)) & 0xF000) >> 12);
    if (printing || nibble != 0 || i == 3) {
      emberAfPrint(emberAfPrintActiveArea,
                   "%c",
...
// This function prints IPv6 addresses per the recommendations of RFC5952.  In
...
void emberAfPrintIpv6Fields(const uint8_t *bytes, uint8_t fields)
{
  uint8_t cStart = MAX_INT8U_VALUE, cLength = 0;
  uint8_t mStart = MAX_INT8U_VALUE, mLength = 0;
  uint8_t i;
  for (i = 0; i < fields; i++) {
    uint16_t field = emberFetchHighLowInt16u(bytes + i * 2);
    if (field == 0) {
      if (cStart == MAX_INT8U_VALUE) {
        cStart = i;
      }
      cLength++;
    } else if (cStart != MAX_INT8U_VALUE) {
      if (mStart == MAX_INT8U_VALUE || mLength < cLength) {
        mStart = cStart;
        mLength = cLength;
      }
      cStart = MAX_INT8U_VALUE;
      cLength = 0;
    }
  }
  if (cStart != MAX_INT8U_VALUE
      && (mStart == MAX_INT8U_VALUE || mLength < cLength)) {
    mStart = cStart;
    mLength = cLength;
  }
  if (mLength == 1) {
    mStart = MAX_INT8U_VALUE;
  }

  for (i = 0; i < fields; i++) {
    if (i < mStart || mStart + mLength <= i) {
      uint16_t field = emberFetchHighLowInt16u(bytes + i * 2);
      if (i != 0) {
        emberAfPrint(emberAfPrintActiveArea, ":");
      }
      printIpv6Field(field);
    } else if (mStart == i) {
      emberAfPrint(emberAfPrintActiveArea, ":");
    }
  }
  if (mStart + mLength == fields) {
    emberAfPrint(emberAfPrintActiveArea, ":");
  }
}
...
{
  emberAfPrintIpv6Fields(address->bytes, sizeof(EmberIpv6Address) / 2);
}

void emberAfPrintIpv6Prefix(const EmberIpv6Prefix *prefix)
{
  emberAfPrintIpv6Fields(prefix->bytes, sizeof(EmberIpv6Prefix) / 2);
}

...
static void doPostRxStateMachineTasks(AshMessageType txType)
{
  ...
  if ((txType == ASH_ACK
    ...
    if ((ashTxState.ackNackFrameCounter == 0 // uninitialized
      ...
      assert(serialCopyFromRx(ashRxState.payload, ashRxState.payloadIndex)
             == payloadIndex);
...
uint8_t emProcessAshRxInput(const uint8_t *data, uint8_t dataLength)
{
  ...
  while (ashRxState.frameState == ASH_DONE) {
    ...
    // process the packet
    doPostRxStateMachineTasks(txType);
    emResetAshRxState();
    txType = ASH_ACK;
...
static void reallyPrintAshState(uint8_t indent)
{
  assert(ashState < COUNTOF(stateStrings));

 ...
 */
  ...
  #ifdef AVR_ATMEGA
  extern uint8_t PGM emSerialTxQueueWraps[EM_NUM_SERIAL_PORTS];
  extern uint8_t PGM emSerialRxQueueWraps[EM_NUM_SERIAL_PORTS];
  #else
...
{
  #ifdef CORTEXM3_EFM32_MICRO
  DEFINE_PARITY(NONE) = usartNoParity,   // PARITY_NONE
  DEFINE_PARITY(ODD) = usartOddParity,   // PARITY_ODD
  DEFINE_PARITY(EVEN) = usartEvenParity  // PARITY_EVEN
  #else //CORTEXM3_EFM32_MICRO
 ...
 */
#ifdef CORTEXM3_EFM32_MICRO
  #define halInternalUartTxIsIdle(port) COM_InternalTxIsIdle(port)
#else

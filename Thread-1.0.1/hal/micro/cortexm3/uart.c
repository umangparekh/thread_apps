...
// and friends decay into a simple register access
  ...
  #endif
#else
  const uint32_t serialControllerBlockAddresses[] = { SC1_RXBEGA_ADDR, SC3_RXBEGA_ADDR };
  ...
  const uint32_t serialControllerIntCfgAddresses[] = { INT_SC1CFG_ADDR, INT_SC3CFG_ADDR };
  #define INT_SCxCFG(port) (*((volatile uint32_t *)serialControllerIntCfgAddresses[(port) - 1]))

  const uint32_t serialControllerIntFlagAddresses[]  = { INT_SC1FLAG_ADDR, INT_SC3FLAG_ADDR };
  #define INT_SCxFLAG(port) (*((volatile uint32_t *)serialControllerIntFlagAddresses[(port) - 1]))

  const uint32_t serialControllerNvecIntValues[] = { INT_SC1, INT_SC3 };
...
#if defined(EM_ENABLE_SERIAL_BUFFER) && !defined (CORTEXM3_EM35X_USB)
...
void halInternalStartUartTx(uint8_t port)
{
    ...
    #endif // defined(EM_ENABLE_SERIAL_BUFFER) && !defined (CORTEXM3_EM35X_USB)
  #endif // EM_PHYSICAL_UART
    ...
    if (EM_SER3_PORT_EN(port)) {
        ...
        usbTxData();
          ...
          while ( q->used > 0 ) {
            ...
            {
              USBD_Write( EP_IN, q->nextByte, (uint32_t)q->lastByte - (uint32_t)q->nextByte + 1, (USB_XferCompleteCb_TypeDef) &usbReleaseBuffer );

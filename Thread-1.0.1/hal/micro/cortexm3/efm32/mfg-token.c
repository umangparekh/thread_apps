                                ...
                                uint8_t len)
{
  ...
  if(token == MFG_EUI_64_LOCATION) {
    ...
    if((realAddress >= LOCKBITS_BASE) && (realAddress < LOCKBITS_BASE+0x200)) {
      //Ignore the lock words.  Perhaps some day they'll be readable here.
      return;
    }
    uint8_t *flash = (uint8_t *)realAddress;
...
void halInternalSetMfgTokenData(uint16_t token, void *data, uint8_t len)
{
  ...
  if((token&0xF000) == (USERDATA_TOKENS&0xF000) ) {
    ...
    flash = (uint8_t *)realAddress;
    //The lock words live between LOCKBITS_BASE to LOCKBITS_BASE+0x1FF.
    //This function should not modify them.
    assert((realAddress>=LOCKBITS_BASE+0x200) && ((realAddress+len-1)<(LOCKBITS_BASE+FLASH_PAGE_SIZE)));
  } else {

...
#define halSystemXtalKHz              24000u
#define halMcuClockKHz() ((wakeupXtalState <= WAKEUP_XTAL_STATE_READY_SWITCH) \
                                    ? 12000u : 24000u)
#define halPeripheralClockKHz()       12000u
...
#define MAC_TIMER_TICKS(microseconds) \
  ((((uint32_t)(microseconds) * (halPeripheralClockKHz() / 250)) + 47) / 48)
...
#define MAC_TIMER_US(ticks) \
  (((((uint32_t)(ticks) * 48000) / (halPeripheralClockKHz() / 250)) + 500) / 1000)
